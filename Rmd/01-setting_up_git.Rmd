...
nu=$(git config --list | grep user.name | wc -l)
ne=$(git config --list | grep user.email | wc -l)

mkdir ~/workdir_tmp

if [ $nu > 0 ];
then
  git config --list | grep user.name | cut -d"=" -f 2 > ~/workdir_tmp/username.txt
fi

if [ $ne > 0 ];
then
  git config --list | grep user.email | cut -d"=" -f 2 > ~/workdir_tmp/email.txt
fi
